...
public partial class SubcategoriaGrid : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        {
            categoria = CntLainsaSci.GetCategoria(int.Parse(Request.QueryString["CategoriaId"]), ctx);
            if (categoria != null)
            {
                RadGrid1.Columns.FindByDataField("Categoria.Nombre").Visible = false;
            }
        }
    }
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            string strKey = item.GetDataKeyValue("SubcategoriaId").ToString();
            ...
            jCommand = String.Format("editSubcategoria('{0}');", strKey);
            imgb.OnClientClick = jCommand;
    ...
    {
        if (categoria == null)
            RadGrid1.DataSource = CntLainsaSci.GetSubcategorias(ctx);
        else
            RadGrid1.DataSource = categoria.Subcategorias;
        if (rebind)
            RadGrid1.Rebind();
    }
    #endregion
