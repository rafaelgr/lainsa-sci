...
public partial class MensajeGrid : System.Web.UI.Page
{
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            string strKey = item.GetDataKeyValue("MensajeId").ToString();
            ...
            jCommand = String.Format("returnValues2('{0}','{1}');", strKey, item["Titulo"].Text);
            ...
            jCommand = String.Format("editMensaje('{0}');", strKey);
            imgb.OnClientClick = jCommand;
            imgb.Visible = permiso.Ver;

            // assign to delete button (not needed by now)
            imgb = (ImageButton)item.FindControl("Delete");
            jCommand = String.Format("return radconfirm('{0}',event,300,100,'','{1}');", Resources.ResourceLainsaSci.DeleteRecordQuestion + " " + item["Titulo"].Text, Resources.ResourceLainsaSci.DeleteRecord);
            imgb.OnClientClick = jCommand;
            imgb.Visible = permiso.Crear;
            if (item["FechaFinal"].Text == "01/01/0001") item["FechaFinal"].Text = "";
            if (item["FechaInicio"].Text == "01/01/0001") item["FechaInicio"].Text = "";
        }
    }
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                ...
                case "Delete":
                    try
                    {
                        Mensaje sc = CntLainsaSci.GetMensaje(id, ctx);
