...
public partial class EstadoGrid : System.Web.UI.Page
{
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            string strKey = item.GetDataKeyValue("EstadoId").ToString();
            ...
            jCommand = String.Format("returnValues3('{0}','{1}','{2}');", strKey, item["Nombre"].Text, "Estado");
            ...
            jCommand = String.Format("editEstado({0});", strKey);
            imgb.OnClientClick = jCommand;
            imgb.Visible = permiso.Ver;

            // assign to delete button (not needed by now)
            imgb = (ImageButton)item.FindControl("Delete");
            jCommand = String.Format("return radconfirm('{0}',event,300,100,'','{1}');", Resources.ResourceLainsaSci.DeleteRecordQuestion + " " + item["EstadoId"].Text, Resources.ResourceLainsaSci.DeleteRecord);
            imgb.OnClientClick = jCommand;
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                ...
                case "Delete":
                    try
                    {
                        Estado estado = CntLainsaSci.GetEstado(id, ctx);
                        CntLainsaSci.CTXEliminar(estado, ctx);
    ...
    {
        RadGrid1.DataSource = CntLainsaSci.GetEstados(usuario, ctx);
        if (rebind)
            RadGrid1.Rebind();
    }
    #endregion
