...
public partial class ProgramaForm : System.Web.UI.Page
{
    ...
    protected void btnPrintFormularios_Click(object sender, ImageClickEventArgs e)
    {
        if (programa != null)
        {
            string url = String.Format("VisorInforme.aspx?ProgramaId={0}&Informe=RptRevisionFormulario", programa.ProgramaId);
            string jCommand = String.Format("openOutSide('{0}','{1}');", url, "VisorInforme");
            RadAjaxManager1.ResponseScripts.Add(jCommand);
        }
    }
    protected void btnPrintInformePlanta_Click(object sender, ImageClickEventArgs e)
    {
        if (programa != null)
        {
            string url = String.Format("VisorInforme.aspx?ProgramaId={0}&Informe=RptInformePlanta", programa.ProgramaId);
            string jCommand = String.Format("openOutSide('{0}','{1}');", url, "VisorInforme");
            RadAjaxManager1.ResponseScripts.Add(jCommand);
        }
    }
