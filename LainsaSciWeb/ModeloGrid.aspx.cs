...
public partial class ModeloGrid : System.Web.UI.Page
{
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        if (ctx != null)
            ctx.Dispose();
        //
        //Session["ModeloDispositivoGrid"] = null;
    }
    #endregion
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridCommandItem)
        {
            ...
            if (tipo != null) 
            {
                img.OnClientClick = String.Format("newModeloTipo({0});", tipo.TipoId);
            }
        }
        if (e.Item is GridDataItem)
        {
            ...
            string strKey = item.GetDataKeyValue("ModeloId").ToString();
            ...
            jCommand = String.Format("returnValues3('{0}','{1}','{2}');", strKey, item["Nombre"].Text, "Modelo");
            ...
            jCommand = String.Format("return radconfirm('{0}',event,300,100,'','{1}');", Resources.ResourceLainsaSci.DeleteRecordQuestion + " " + item["ModeloId"].Text, Resources.ResourceLainsaSci.DeleteRecord);
            imgb.OnClientClick = jCommand;
    ...
    protected void RefreshGrid(bool rebind)
    {
        if (tipo != null)
        {
            RadGrid1.DataSource = tipo.ModelosDispositivo;
        }
        else
        {
            if (usuario == null)
                RadGrid1.DataSource = CntLainsaSci.GetModeloDispositivos(ctx);
            else
                RadGrid1.DataSource = CntLainsaSci.GetModeloDispositivos(usuario, ctx);
        }
        if (rebind)
            RadGrid1.Rebind();
    }
