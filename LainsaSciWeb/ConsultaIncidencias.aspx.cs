...
public partial class ComsultaIncidencias : System.Web.UI.Page
{
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                case "Select":
                    break;
                case "Edit":
                    string url = String.Format("IncidenciaForm.aspx?IncidenciaId={0}&Modo=S", id);
                    string jCommand = String.Format("openOutSide('{0}', 'IncidenciaForm');", url);
    ...
    protected void RefreshGrid(bool rebind)
    {
        if (usuario != null)
            ...
            RadGrid1.DataSource = CntLainsaSci.GetIncidencias(ctx);
        if (rebind)
            RadGrid1.Rebind();
    }
    ...
    protected void CargarFiltros(Filtro fil)
    {
        ...
        foreach (Filtro f in CntLainsaSci.GetFiltros("Incidencia", usuario, ctx))
        {
    ...
    protected void btnInforme_Click(object sender, EventArgs e)
    {
        ...
        {
            incidencias.Add(item["IncidenciaId"].Text);
        }
        Session["Filtro"] = incidencias;
    ...
    protected void RadButton1_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridDataItem item in RadGrid1.MasterTableView.Items)
        {
            incidencias.Add(item["RevisionId"].Text);
        }
        Session["Filtro"] = incidencias;
        RadGrid1.AllowPaging = true;// disabling paging
        RadGrid1.Rebind();
