...
public partial class EmailGrid : System.Web.UI.Page
{
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                ...
                case "Delete":
                    try
                    {
                        Email em = CntLainsaSci.GetEmail(id, ctx);
                        CntLainsaSci.CTXEliminar(em, ctx);
    ...
    {
        if (empresa != null)
            RadGrid1.DataSource = empresa.Emails;
        if (instalacion != null)
            RadGrid1.DataSource = instalacion.Emails;
        if (rebind)
            RadGrid1.Rebind();
    }
    #endregion
    ...
    protected void ControlDeError(Exception ex)
    {
        ...
        eC.ErrorProcess = new Proceso() { Nombre = "DIR", Descripcion = "Email" };
