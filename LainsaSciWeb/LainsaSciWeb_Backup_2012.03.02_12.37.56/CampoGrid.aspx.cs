...
public partial class CampoGrid : System.Web.UI.Page
{
    ...
    protected void Page_Unload(object sender, EventArgs e)
    {
        ...
        if (ctx != null)
            ctx.Dispose();
        //
        //Session["CampoDispositivoGrid"] = null;
    }
    #endregion
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridCommandItem)
        {
            ...
            if (plantilla != null)
            {
                img.OnClientClick = String.Format("newCampoFromPlantilla('{0}');", plantilla.PlantillaId);
            }
            if (mode.Equals("S"))
            {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            string strKey = item.GetDataKeyValue("CampoId").ToString();
            ...
            jCommand = String.Format("editCampo({0});", strKey);
            if (plantilla != null)
                jCommand = String.Format("editCampoFromPlantilla({0});", strKey);
            imgb.OnClientClick = jCommand;
            imgb.Visible = permiso.Ver;

            // assign to delete button (not needed by now)
            imgb = (ImageButton)item.FindControl("Delete");
            jCommand = String.Format("return radconfirm('{0}',event,300,100,'','{1}');", Resources.ResourceLainsaSci.DeleteRecordQuestion + " " + item["CampoId"].Text, Resources.ResourceLainsaSci.DeleteRecord);
            imgb.OnClientClick = jCommand;
    ...
    {
        if (plantilla != null)
            RadGrid1.DataSource = plantilla.Campos;
        //else
        //    RadGrid1.DataSource = CntLainsaSci.GetCampos(ctx);
